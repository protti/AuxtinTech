...
public class Admin extends Client {
	...
	public Admin(String ssn, String name, String surname, int age, String birthdate, String password, String number, String city, String street, int civic, String cap, int type_abb, boolean expired, double to_pay, double paid, String dataSub)
	{
	...
	public void saveToDB() throws SQLException
	{
		...
		{
			Statement st = con.createStatement();
			st.executeUpdate("insert into admin values('" + super.getSsn() + "'," + this.to_pay + "," + this.paid + "," + this.type_abb + "," + this.expired + ",'" + this.dataSub + "')");
		}
	}
	...
	public void updateAll() throws SQLException
	{
		...
		{
			Statement st = con.createStatement();
			st.executeUpdate("update admin set to_pay = " + this.to_pay + ", paid = " + this.paid +", type_abb = " + this.type_abb + ", scad = " + this.expired + ", dataCreazione = '" + this.dataSub + "' where ssn = '" + super.getSsn() + "'");
		}
	}
	...
	public void removeProduct(String codiceID) throws SQLException
	{
		if(this.isSave())
		{
	...
	public void payInterest(Card c) throws SQLException,ProductNotSaldableException
	{
		if(this.isSave() && c.isSave() && c.getSsn().equals(super.getSsn()) && c.getSald() >= this.to_pay)
		{
	...
	public boolean myProduct(Material m) throws SQLException
	{
		if(this.isSave() && m != null)
		{
			...
			if(rs.next()) return this.getSsn().equals(rs.getString(1));
			else return false;
		}
		else return false;
	}
	
	public ArrayList<Material> getMyProducts() throws SQLException
	{
		if(this.isSave())
		{
			Statement st = con.createStatement();
			ResultSet rs = st.executeQuery("select codiceID from inserting where ssn = '" + this.getSsn() + "'");
			ArrayList<Material> cartAdmin = new ArrayList<Material>();
			while(rs.next())
			{
				
				cartAdmin.add(this.searchMat(rs.getString(1)));
			}
	...
	public ArrayList<Feedback> myFeedback() throws SQLException
	{
		if(this.isSave())
		{
			ArrayList<Feedback> fd = new ArrayList<Feedback>();
			Statement st = con.createStatement();
			ResultSet rs = st.executeQuery("select * from feedback where assn = '" + super.getSsn() + "'");
			while(rs.next())
			{
				Feedback f = new Feedback(rs.getString(2),rs.getString(4),rs.getString(3),rs.getLong(1));
				if(f.isSave()) fd.add(f);
			}
			return fd;
		}
		return null;
